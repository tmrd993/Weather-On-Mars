...
import org.timucin.backend.service.DataRetrieverService;
...
public class MainView extends VerticalLayout {
	 ...
	 */
	public MainView() {
		dataRetrieverService = getDataRetrieverService();
		
		H1 title = new H1("Weather on Mars");
		
		List<SolTemperatureData> temperatureData = dataRetrieverService.fetchCurrentWeatherData();
	...
	private DataRetrieverService getDataRetrieverService() {
		return (DataRetrieverService) WebApplicationContextUtils
				.getWebApplicationContext(VaadinServlet.getCurrent().getServletContext())
				.getBean(DataRetrieverService.class);
	}
}
